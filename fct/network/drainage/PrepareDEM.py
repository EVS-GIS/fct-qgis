...
class PrepareDEM(AlgorithmMetadata, QgsProcessingAlgorithm):
    ...
    def initAlgorithm(self, configuration):
        ...
        self.addParameter(QgsProcessingParameterNumber(
            self.WINDOW,
            self.tr('Smoothing window (in pixels)'),
            defaultValue=5,
            minValue=1))
    ...
    def processAlgorithm(self, parameters, context, feedback): 
        ...
        dem_tiles = TileDataset(dem_layer, tileset, directory=config['output_dir'], feedback=feedback, overwrite=config['overwrite'])
        ...
        output_meanfilter = [(t[0], t[1], t[2], t[3].output) for t in tasks if t[3].output and os.path.exists(t[3].output)]

        if output_meanfilter:
            tiles_paths = [tile[3] for tile in output_meanfilter]
